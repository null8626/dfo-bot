...
try {
  logger.info(`Attempting to register ${commandArray.length} server commands...`);
...
try {
  logger.info(`Attempting to register ${globalCommandArray.length} global commands...`);
  rest.put(Routes.applicationCommands(process.env.APP_ID!), { body: globalCommandArray });
  logger.info('Successfully registered commands!');
} catch (err) {
  throw err;
}