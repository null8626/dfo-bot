...
export default class NetworkCommand extends SlashCommand {
  ...
  public async execute(interaction: ChatInputCommandInteraction, client: Client): Promise<void> {
    ...
    switch (choice) {
      case 'overview': {
        ...
        const avgPing = clusters.reduce((acc, s) => acc + s.ping, 0) / clusters.length;
      ...
      case 'shard': {
        ...
        if (id.toLowerCase() === 'all') {
          ...
          for (let i = 0; i < clusters.length; i += ITEMS_PER_PAGE) {
            ...
            pages.push(new EmbedBuilder().setColor(Colors.Blue).setTitle('Network Manager: Clusters').setDescription(descriptionText));
          }
          ...
          if (!targetCluster) {
            await interaction.editReply({ content: `❌ No cluster could be found with the ID: \`${id}\`` });
            return;
          }
  ...
  private async getClusters(client: Client): Promise<ClusterInfo[]> {
    ...
    if (cluster) {
      const results: ClusterInfo[] = await cluster.broadcastEval((c: any) => ({
        ...
        users: c.guilds.cache.reduce((acc: number, guild: any) => acc + guild.memberCount, 0),
        uptime: c.uptime
      }));
      return results;
    } else {
  ...
  private async getGuilds(client: Client): Promise<GuildInfo[]> {
    const cluster = (client as any).cluster;
    if (cluster) {
      const results: GuildInfo[][] = await cluster.broadcastEval((c: any) => 
        c.guilds.cache.map((g: any) => ({
          id: g.id,
          name: g.name,
