import { AutocompleteInteraction, ChatInputCommandInteraction, Client, Colors, EmbedBuilder, MessageFlags } from "discord.js";
...
import { IScenarioJSON } from "../interfaces/IScenarioJSON";
import ScenarioLookupContainer from "../structures/containers/ScenarioLookupContainer";
import { INPCJSON } from "../interfaces/INPCJSON";
...
export default class LookupCommand extends SlashCommand {
  ...
  public async autocomplete(interaction: AutocompleteInteraction, client: Client): Promise<void> {
    ...
    const matches = items
      .filter(item => item.name.toLowerCase().includes(query) || String(item.itemId).startsWith(query))
      .slice(0, 25)
      .map(item => ({
      ...
      case 'item':
        if (id === -1) {
          ...
          for (let i = 0; i < items.length; i += ITEMS_PER_PAGE) {
            ...
            pages.push(new EmbedBuilder().setColor(Colors.Green).setTitle('Item Manager').setDescription(descriptionText));
          }
          ...
          if (!item) {
            await interaction.editReply({ content: 'No item with that id exists!' });
            return;
          }
          await interaction.editReply({ components: [new ItemLookupContainer(item).build()], flags: MessageFlags.IsComponentsV2 });
        }
        break;
      ...
      case 'scenario':
        if (id === -1) {
          ...
          for (let i = 0; i < data.length; i += ITEMS_PER_PAGE) {
            ...
            pages.push(new EmbedBuilder().setTitle('Scenario Manager').setDescription(descriptionText));
          }
          ...
          if (res.status === 404) { await interaction.editReply({ content: 'No scenario was found for this id!' }); return; }
          if (!res.ok) throw new Error('API Error!');
          const { data }: { data: IScenarioJSON } = await res.json();
          await interaction.editReply({ components: [new ScenarioLookupContainer(data).build()], flags: MessageFlags.IsComponentsV2 });
        }
        break;
      ...
      case 'npc':
        if (id === -1) {
          ...
          for (let i = 0; i < data.length; i += ITEMS_PER_PAGE) {
            ...
            pages.push(new EmbedBuilder().setTitle('NPC Manager').setDescription(descriptionText));
          }
          ...
          if (!res.ok) throw new Error('API Error!');
          const { success, data }: { success: boolean, data: INPCJSON } = await res.json();
          if (!success) { await interaction.editReply({ content: 'No NPC was found for the provided ID!' }); return; }
          await interaction.editReply({ components: [new NPCLookupContainer(data).build()], flags: MessageFlags.IsComponentsV2 });
        }
        break;
    }
  }
