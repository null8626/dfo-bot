  ...
  public async execute(
    ...
    try {
      ...
      if (!res.ok || !body.success) {
        await interaction.editReply({
          content: formatError(body.error ?? 'Failed to open chest'),
          files: [],
          components: [],
          embeds: []
        });
      ...
      for (const item of loot.items) {
        ...
        lines.push(
          `${emoji} **${item.name}** — ${item.rarity} ${item.type} (Lvl ${item.level})`
        );
      }
      ...
      lines.push(``);
      if (loot.gold > 0)
        lines.push(`🪙 **+${loot.gold.toLocaleString()}** Gold`);
      if (loot.embers > 0)
        lines.push(`🔥 **+${loot.embers.toLocaleString()}** Embers`);
