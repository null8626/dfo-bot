...
export default class MarketPrevButton extends Button {
  constructor() {
    super({ customId: 'mkt_prev', cooldown: 2, isAuthorOnly: true });
  }
 ...
 */
export async function handleMarketPage(
  interaction: ButtonInteraction,
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor(mode === 'my_listings' ? 0x3b82f6 : 0x10b981)
    ...
    if (listings.length > 0) {
      ...
      for (
        let rowStart = 0;
        rowStart < listings.length && rows.length < 2;
        rowStart += 4
      ) {
        const row = new ActionRowBuilder<ButtonBuilder>();
        for (
          let j = rowStart;
          j < Math.min(rowStart + 4, listings.length);
          j++
        ) {
          ...
          row.addComponents(
            new ButtonBuilder()
              .setCustomId(
                `${isBrowse ? 'mkt_buy' : 'mkt_cancel'}:${listing.listingId}`
              )
