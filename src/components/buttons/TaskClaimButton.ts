  ...
  public async execute(
    ...
    if (!taskId) {
      await interaction.editReply({
        content: 'Error parsing task data!',
        files: [],
        components: [],
        embeds: []
      });
    ...
    try {
      ...
      if (!res.ok || !body.success) {
        await interaction.editReply({
          content: formatError(body.error ?? 'Failed to claim task'),
          files: [],
          components: [],
          embeds: []
        });
      ...
      const lines = [`✅ **Task Claimed!**`, ``];

      if (reward) {
        if (reward.gold > 0)
          lines.push(`🪙 **+${reward.gold.toLocaleString()}** Gold`);
        if (reward.xp > 0)
          lines.push(`✨ **+${reward.xp.toLocaleString()}** XP`);
        if (reward.embers > 0)
          lines.push(`🔥 **+${reward.embers.toLocaleString()}** Embers`);
      }

      if (body.levelsGained > 0) {
        lines.push(
          ``,
          `🆙 **Gained ${body.levelsGained} Level${body.levelsGained > 1 ? 's' : ''}!**`
        );
      }
