 ...
 */
export function formatCooldown(action: 'step' | 'combat', cooldownRemainingMs?: number): string {
  if (cooldownRemainingMs) {
    const futureTimestamp = Math.floor(Date.now() / 1000) + Math.ceil(cooldownRemainingMs / 1000);
    if (action === 'step') {
  ...
  if (action === 'step') return '⏳ **Recovering...** Please wait before exploring again.';
