...
export default class ItemImageBuilder {
  public static async build(item: IItemJSON): Promise<Buffer> {
    // Dynamically calculate canvas height based on how many affixes there are
    const affixesCount = item.affixes?.length || 0;
    
    // CRITICAL FIX: Increased base padding from 380 to 430 so the footer sits cleanly below the boxes
    ...
    // Outer Glow / Border
    ctx.lineWidth = 4;
    ctx.strokeStyle = `${color}44`; // 44 is hex for ~26% opacity
    ctx.strokeRect(2, 2, canvas.width - 4, canvas.height - 4);

    const bgGradient = ctx.createLinearGradient(0, 0, 0, 150);
    bgGradient.addColorStop(0, `${color}11`); // Slight tint at the top
    ...
    ctx.fillText(item.name, canvas.width / 2, 135, 560);

    // 3. Badges (Rarity & Type)
    ...
    let currentX = (canvas.width - (rWidth + tWidth + 10)) / 2;
    ...
    if (item.type === 'Consumable') {
        // Consumable Effect Box
        ctx.fillStyle = '#713f1233'; // yellow-900/20
        ctx.strokeStyle = '#eab3084d'; // yellow-500/30
        ctx.beginPath();
    ...
    if (affixesCount > 0) {
        ...
        item.affixes!.forEach(affix => {
            ctx.fillStyle = '#581c8733'; // purple-900/20
            ctx.strokeStyle = '#a855f733'; // purple-500/20
            ctx.beginPath();
